...
def get_wheel_state():
    ...
    try:
        ...
        resp_fl = gls('p3at_front_left_wheel', 'base_link')
        resp_fr = gls('p3at_front_right_wheel', 'base_link')
        resp_bl = gls('p3at_back_left_wheel', 'base_link')
        resp_br = gls('p3at_back_right_wheel', 'base_link')
        return resp_fl, resp_fr, resp_bl, resp_br
    except rospy.ServiceException as e:
        rospy.logerr("Service call failed: %s"%e)
        return False
...
def is_pose_in_map(pose):
    ...
    #     return False
    if pose.position.z < -3.0: # beacuse of urdf confituration, the z 0 is ~ -0.02
        return False
    return True

def inImageFrame(pix, min_pix, max_pix):
    if pix < min_pix: 
        pix = math.ceil(min_pix)
    elif pix > max_pix:
        pix = math.floor(max_pix)
    return pix

def toImageFrame(s_x, s_y, s_z, im_info):

    # Converting from sim coords to image coords
    xyz_simulation = np.array([s_x, s_y, s_z])
    offset = np.array([im_info["CAM_POS_X"],im_info["CAM_POS_Y"],im_info["CAM_POS_Z"]])
    xyz_unoffset = xyz_simulation - np.array([offset[0][0], offset[1][0], offset[2][0]])

    # Using camera constant to calculate x pixels(horrizontal) and z pixels (vertical) 
    x_pix_delta = im_info["CAM_CONSTANT"]*(xyz_unoffset[0]/xyz_unoffset[1])
    z_pix_delta = im_info["CAM_CONSTANT"]*(xyz_unoffset[2]/xyz_unoffset[1])
    x_pix_centre = im_info["IM_SIZE_X"]/2
    z_pix_centre = im_info["IM_SIZE_Z"]/2

    # Pixels from top left corner of image
    x_pix = x_pix_centre + x_pix_delta
    x_pix = inImageFrame(x_pix[0], 0, im_info["IM_SIZE_X"][0]-1)

    z_pix = z_pix_centre - z_pix_delta
    z_pix = inImageFrame(z_pix[0], 0, im_info["IM_SIZE_Z"][0]-1)

    return x_pix, z_pix

# Find terrain friction from at image
def toImageFriction(x_pix, z_pix, im_GT):
    # x_pix = horrizontal, z_pix = vertical 
    pix_val = im_GT[int(round(z_pix)), int(round(x_pix))] # Assumes that pixel has been checked if in image already 
    if np.array_equal(pix_val,[170,170,170]):
        # print "Road"
        pix_mu = 1.0
    elif np.array_equal(pix_val,[0,255,0]):
        # print "Grass"
        pix_mu = 0.75
    elif np.array_equal(pix_val,[102,102,51]):
        # print "Vegetation"
        pix_mu = 0.5
    elif np.array_equal(pix_val,[0,60,0]):
        # print "Tree"
        pix_mu = 0.25
    elif np.array_equal(pix_val,[0,120,255]):
        # print "Sky"
        pix_mu = 0.001
    elif np.array_equal(pix_val,[0,0,0]):
        # print "Obstacle"
        pix_mu = 0.001
    else: 
        # print "Void"
...
# Find terrain class at pixel from ground truth image
def toImageClass(x_pix, z_pix, im_GT):
    # x_pix = horrizontal, z_pix = vertical 
    pix_val = im_GT[int(round(z_pix)), int(round(x_pix))] # Assumes that pixel has been checked if in image already 
...
class PubsSubsManager:
    def __init__(self):
        ...
        # Friction values
        self.mu_ = 1.0 # Initial friction 
        self.cmd_mu_topic_ = "/gazebo/cmd_mu"
        self.pub_cmd_mu_ = rospy.Publisher(self.cmd_mu_topic_, Float32, queue_size=10)
    ...
    def publish_cmd_mu(self):
        msg = self.mu_
        self.pub_cmd_mu_.publish(msg)
    ...
    def callback_odom(self, msg):
        ...
        if self.process_odom_:
            gms = get_model_state()
            # fl, fr, bl, br = get_wheel_state()
            # print(fl)
            if is_pose_in_map(msg.pose[model_index]) and gms.success:
                
                # Calculate terrain friction value
                x_pix, z_pix = toImageFrame(gms.pose.position.x,
                                            gms.pose.position.y,
                                            gms.pose.position.z,
                                            im_info)
                new_mu = toImageFriction(x_pix,z_pix,im_GT)

                # Publish terrain friction value (if it has been updated)
                if new_mu != self.mu_: 
                    terrain_class = toImageClass(x_pix,z_pix,im_GT)
                    print "Terrain class = ", terrain_class
                    self.mu_ = new_mu
                    self.publish_cmd_mu()
    ...
    def stop_simulation_try(self):
        ...
        rospy.loginfo("saving simulation data to %s",output_folder+self.world_name_+"_"+str(self.i_sim_)+".csv")
        self.data_buffer_.to_csv(output_folder+self.world_name_+"_"+str(self.i_sim_)+".csv")
        return self.world_name_+"_"+str(self.i_sim_)+".csv"
...
if __name__ == "__main__":
    ...
    im_GT = skimage.io.imread(world_GT)
    ...
    # Removed as all image info was just copied across from meshes folder
    # # Save mesh image
